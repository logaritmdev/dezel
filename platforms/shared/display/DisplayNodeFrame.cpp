...
DisplayNodeFrame::DisplayNodeFrame(DisplayNode* node) : layoutResolver(node)
{
	this->node = node;
}
...
void
DisplayNodeFrame::setContentDisposition(DisplayNodeContentDisposition location)
{
...
{
	if (this->node->type == kDisplayNodeTypeRoot) {
		return false;
	}
...
DisplayNodeFrame::measure()
{
	...
	if (frame->isAbsolute()) {
		...
		frame->layoutResolver.measureAbsoluteNode(this->node);

	} else {

		const double paddingT = this->node->parent->frame->measuredPaddingTop;
		const double paddingL = this->node->parent->frame->measuredPaddingLeft;
		const double paddingR = this->node->parent->frame->measuredPaddingRight;
		const double paddingB = this->node->parent->frame->measuredPaddingBottom;

		double remainder = 0;
		double remainingW = max(this->node->parent->frame->measuredContentWidth - paddingL - paddingR, 0.0);
		double remainingH = max(this->node->parent->frame->measuredContentHeight - paddingT - paddingB, 0.0);

		frame->layoutResolver.measureRelativeNode(this->node, remainingW, remainingH, remainder);
	}
...
DisplayNodeFrame::resolveWrapper(double w, double h)
{
	...
	if (hasIntrinsicW == false ||
		...
		if (wrapW) w = this->layoutResolver.getExtentRight() - this->layoutResolver.getExtentLeft();
		if (wrapH) h = this->layoutResolver.getExtentBottom() - this->layoutResolver.getExtentTop();
