 ...
 * @since 0.4.0
 */
open class ListOptimizer(context: JavaScriptContext) : ContentOptimizer(context) {
	 ...
	 * @hidden
	 */
	private var list: MutableList<View> = mutableListOf()
	 ...
	 * @hidden
	 */
	private fun createItem(index: Int): View? {
		...
		val item = view.property("native").cast(View::class.java)
		if (item == null) {
			return null
		}
	 ...
	 * @hidden
	 */
	private fun reuseItem(index: Int): View? {
	 ...
	 * @hidden
	 */
	private data class InsertItemData(val view: View, val index: Int)
	 ...
	 * @hidden
	 */
	private data class RemoveItemData(val view: View, val index: Int)
}
